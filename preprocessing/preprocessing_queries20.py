from ColBERT.paths import *
import ast
import os
import pickle
from transformers import BertTokenizerFast
from argparse import ArgumentParser
# from preprocessing_queries import remove_prefix
import re
...
def main(args):
    ...
    # Load canonical passages pickle
    with open(path_queries['cast20']['canonical_passage_pickle'],'rb') as f:
        canonical_passages = pickle.load(f)

    with open(path_queries['cast20']['json']) as f:
        data = ast.literal_eval(f.read())

    queries = dict()
    tokenizer = BertTokenizerFast.from_pretrained('bert-large-uncased')

    # Set paths & remove previously written
    output_path = path_queries['cast20'][f'full_conv_canonical_{args.max_len}']

    if args.only_hist:
        output_path = path_queries['cast20'][f'history_only']

    elif args.only_canonical:
        output_path = path_queries['cast20'][f'canonical_only']
    ...
    for entry in data:
        ...
        for turn in entry['turn']:
            ...
            queries[conv_id][turn_id] = {
                ...
                'passage_answer': canonical_passages[turn['automatic_canonical_result_id']],
            }
    ...
    for conv_id, q in queries.items():
        for turn_id, turn in q.items():
            ...
            qid_mapping[qid] = f"{conv_id}_{turn_id}"
    ...
    nr_queries_written = len(open(output_path).readlines(  ))
    nr_qid_mappings = len(qid_mapping)
    assert nr_qid_mappings==nr_queries_written , "nr qids in mapping do not match nr qids in query file"

    # Export qid mapping
    with open(path_queries['cast20']['qid_mapping'],'wb') as f:
        print(f"Writing {len(qid_mapping)} qid mappings in {f.name}")
        pickle.dump(qid_mapping, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_len', dest='max_len', default=256, type=int)
